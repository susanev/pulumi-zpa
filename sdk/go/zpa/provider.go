...
func NewProvider(ctx *pulumi.Context,
	...
	if isZero(args.ZpaClientId) {
		args.ZpaClientId = pulumi.StringPtr(getEnvOrDefault("", nil, "ZPA_CLIENT_ID").(string))
	}
	if isZero(args.ZpaClientSecret) {
		args.ZpaClientSecret = pulumi.StringPtr(getEnvOrDefault("", nil, "ZPA_CLIENT_SECRET").(string))
	}
	if isZero(args.ZpaCloud) {
		args.ZpaCloud = pulumi.StringPtr(getEnvOrDefault("", nil, "ZPA_CLOUD").(string))
	}
	if isZero(args.ZpaCustomerId) {
		args.ZpaCustomerId = pulumi.StringPtr(getEnvOrDefault("", nil, "ZPA_CUSTOMER_ID").(string))
	}
